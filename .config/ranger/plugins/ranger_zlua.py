...
if not ZLUA_LUAEXE:
    _report_error(
        "Please install lua in $PATH or make sure $ZLUA_LUAEXE points to a lua executable.\n"
    )
if (not ZLUA_SCRIPT) or (not os.path.exists(ZLUA_SCRIPT)):
    _report_error(
        "Could not find z.lua, please make sure $ZLUA_SCRIPT is set to absolute path of z.lua.\n"
    )
            ...
            if mode:
                ...
                if mode in ("-e", "-x"):
                    path = subprocess.check_output(
                        [ZLUA_LUAEXE, ZLUA_SCRIPT, "--cd"] + args
                    )
